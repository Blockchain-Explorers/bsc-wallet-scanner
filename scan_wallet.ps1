<#
================================================================================
 BSC (BNB Smart Chain) Multi-Wallet Live Transfer Scanner
================================================================================
 Purpose:
 Monitors one or more BSC wallet addresses for incoming and outgoing
 transfers (both Native BNB and BEP-20 Tokens) in near real-time without
 holding open a CMD prompt.

 Architecture & Features:
 - Background Tray App: Lives in the Windows System Tray (BSC logo icon).
 - Transfer Detection: Compares block-by-block balance changes and decodes
   BEP-20 Transfer events to filter out non-transfer transactions.
...
$form.Text = "BSC Wallet Transfer Monitor"
...
$lblTitle.Text = "BSC Live Transfer Monitor"
$lblTitle.Font = New-Object System.Drawing.Font("Segoe UI", 12, [System.Drawing.FontStyle]::Bold)
$lblTitle.ForeColor = [System.Drawing.Color]::FromArgb(243, 186, 47)   # BNB yellow
...
# ------------------------------------------------------------------------------
# 5. BSC JSON-RPC Communication & Transfer Parsing
# ------------------------------------------------------------------------------

# Tracks the last processed block number per wallet (hex string)
$LastBlock = @{}

# BEP-20 Transfer topic: keccak256("Transfer(address,address,uint256)")
# Identical to ERC-20 — BEP-20 is a fork of ERC-20
...
function Invoke-BscRpcWithRetry {
...
function Get-LatestBlockHex {
    $res = Invoke-BscRpcWithRetry -Payload @{
...
# Get BNB balance of an address at a given block (hex), returned in BNB
function Get-BnbBalance {
...
# Retrieve BEP-20 Transfer logs in a block range where the watched wallet is sender OR receiver
function Get-Bep20Transfers {
    ...
    $logsFrom = @()
    $resFrom = Invoke-BscRpcWithRetry -Payload @{
        jsonrpc = "2.0"; id = 1; method = "eth_getLogs"
        params  = @(@{
            fromBlock = $FromBlockHex
            toBlock   = $ToBlockHex
            topics    = @($BEP20_TRANSFER_TOPIC, $paddedAddr, $null)
        })
    }
    ...
    $logsTo = @()
    $resTo = Invoke-BscRpcWithRetry -Payload @{
        jsonrpc = "2.0"; id = 1; method = "eth_getLogs"
        params  = @(@{
            fromBlock = $FromBlockHex
            toBlock   = $ToBlockHex
            topics    = @($BEP20_TRANSFER_TOPIC, $null, $paddedAddr)
        })
    }
...
# Decode a hex uint256 data field (BEP-20 Transfer amount) to a human-readable value.
...
# Shorten a BSC address: 0x1234...abcd
function Short-Address {
    param([string]$Addr)
    return $Addr.Substring(0, 6) + "..." + $Addr.Substring($Addr.Length - 4)
}

# Core scan: detect native BNB changes and BEP-20 transfers for one wallet over a block range
function Check-Transfers {
    ...
    # (A) Native BNB balance change
    try {
        $balBefore = Get-BnbBalance -Address $TargetWallet -BlockHex $prevBlock
        $balAfter  = Get-BnbBalance -Address $TargetWallet -BlockHex $ToBlockHex
...
$trayIcon.ShowBalloonTip(750, "BSC Monitor Ready", "Watching $($WatchedList.Count) wallet(s) in background.", [System.Windows.Forms.ToolTipIcon]::Info)
