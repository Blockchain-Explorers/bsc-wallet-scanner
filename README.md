# BSC Wallet Transfer Scanner

A lightweight, zero-dependency Windows desktop tool to monitor one or more BSC (BNB Smart Chain) wallet addresses in near real-time for incoming and outgoing transfers (both Native BNB and BEP-20 Tokens).
...
- **Transfers-Only Filtering**: Detects real balance changes (Native BNB & BEP-20 Tokens) by comparing per-block balances and decoding `Transfer` event logs, ignoring unrelated contract calls.
- **System Tray Integration**: Lives in the taskbar notification area with a custom BSC icon.
...
├── bsc.ico            # Custom BSC tray and application icon
...
Open `wallets.txt` and add the BSC wallet addresses you want to watch (one per line):

```text
# Add one BSC wallet address per line
...
- The BSC icon will appear in your **System Tray** (bottom-right next to the clock).
...
| Action | How-To |
| :--- | :--- |
| **View Live Logs** | **Double-click** the BSC tray icon (or right-click -> *Show / Hide Logs*). |
...
# Use a custom RPC provider (QuickNode, Ankr, GetBlock, etc.)
.\scan_wallet.ps1 -RpcUrl "https://bsc-mainnet.g.alchemy.com/v2/YOUR_KEY"
```

> The default public RPC endpoint is `https://bsc-dataseed.binance.org`. For higher rate limits and reliability, a private RPC key from QuickNode or Ankr is recommended.
...
1. Calls `eth_blockNumber` to get the current BSC chain head.
2. For each watched wallet, compares the BNB balance at the previous block vs. the current block (`eth_getBalance`) to detect native BNB movements.
3. Calls `eth_getLogs` for BEP-20 `Transfer(address,address,uint256)` events where the wallet appears as sender or receiver in the indexed topics.
4. Fires a desktop balloon notification and audio chime for every detected movement.

> BSC uses the same EVM-compatible JSON-RPC interface as Ethereum, so `eth_blockNumber`, `eth_getBalance`, and `eth_getLogs` work identically on both chains.
